...
function anatta:loadSystem(moduleScript)
	...
	if system.pure then
		...
		-- events
		self:_initializeBaseSystem(system, Entity.Reducer.new(registry, {
		...
		self:_initializeBaseSystem(system, connections, registry, selector)
	end
end
...
function anatta:_initializeBaseSystem(system, connections, ...)
	if system.init then
		system.init(...)
	end

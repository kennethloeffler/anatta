...
local ValidationListener = require(script.Parent.Generic.ValidationListener)
	...
	local function loadDefinitions(moduleScript)
		...
		for componentName, typeDefinition in pairs(componentDefinitions) do
			...
			local success, attributeMap =
				util.tryToAttribute(default, componentName, typeDefinition)
			...
			ValidationListener.init(system, registry, componentName)
